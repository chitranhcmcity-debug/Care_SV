...
router.post('/call-advice', verifyToken, requireStaffOrAdmin, async (req, res, next) => {
  try {
    ...
    const absentCount = history.filter((a) =>
      a.absentStudents.some((id) => String(id) === String(student._id)),
    ).length;

    const priorCallTasks = await CallTask.find({ studentId: student._id })
      .sort({ createdAt: -1 })
      .limit(5)
  ...
  try {
    ...
    const [totalStudents, totalCourseGroups, totalStaff, callStats, taskStats, settings] =
      await Promise.all([
        ...
        CallTask.aggregate([{ $group: { _id: '$status', count: { $sum: 1 } } }]),
...
- Nhiệm vụ gọi điện theo trạng thái: ${callStats.map((s) => `${s._id}: ${s.count}`).join(', ') || 'chưa có'}
- Nhiệm vụ nội bộ theo trạng thái: ${taskStats.map((s) => `${s._id}: ${s.count}`).join(', ') || 'chưa có'}
...
router.post('/staff-performance', verifyToken, requireAdmin, async (req, res, next) => {
  try {
    ...
    const [callTasks, workTasks] = await Promise.all([
      CallTask.find({ assignedStaffId: staffId }).select('status callAttempts'),
      Task.find({ assignedTo: staffId }).select('status'),
    ]);

    const callByStatus = callTasks.reduce((acc, t) => {
      acc[t.status] = (acc[t.status] || 0) + 1;
      return acc;
    }, {});
    const avgCallAttempts = callTasks.length
      ? (callTasks.reduce((sum, t) => sum + (t.callAttempts || 0), 0) / callTasks.length).toFixed(1)
      : '0';
    const workByStatus = workTasks.reduce((acc, t) => {
      acc[t.status] = (acc[t.status] || 0) + 1;
      return acc;
    }, {});
...
Thống kê nhiệm vụ gọi điện chăm sóc sinh viên (tổng ${callTasks.length} nhiệm vụ):
${Object.entries(callByStatus).map(([k, v]) => `- ${k}: ${v}`).join('\n') || '(chưa có)'}
Số lần gọi trung bình mỗi nhiệm vụ: ${avgCallAttempts}

Thống kê nhiệm vụ nội bộ được giao (tổng ${workTasks.length} nhiệm vụ):
${Object.entries(workByStatus).map(([k, v]) => `- ${k}: ${v}`).join('\n') || '(chưa có)'}
...
Không suy diễn nguyên nhân cá nhân, chỉ nhận xét dựa trên số liệu khách quan. Trả lời tiếng Việt, dưới 120 từ.`;
    ...
    res.json({
      assessment,
      stats: { callTasks: callByStatus, workTasks: workByStatus, avgCallAttempts },
    });
  } catch (error) {
    next(error);
  }
