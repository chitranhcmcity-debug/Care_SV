...
// =========================================================
router.get('/course-template', async (req, res) => {
  try {
    ...
    console.error('Export course template error:', error);
    res.status(500).json({ message: 'Lỗi khi xuất file Excel mẫu theo học phần' });
  }
});
...
// =========================================================
router.post('/import-by-course', verifyToken, requireStaffOrAdmin, upload.single('file'), async (req, res) => {
  try {
    ...
    for (const worksheet of workbook.worksheets) {
      ...
      if (!courseGroup) {
        errors.push(`Sheet "${sheetName}": Không tìm thấy học phần với mã "${sheetName}" trong hệ thống. Bỏ qua.`);
        continue;
      }
    ...
    console.error('Import by course error:', error);
    res.status(500).json({ message: 'Lỗi khi xử lý file Excel import theo học phần: ' + error.message });
  }
});
...
router.get('/export-template', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Lỗi khi xuất file Excel mẫu' });
  }
});
...
// =========================================================
router.post('/import-data', verifyToken, requireStaffOrAdmin, upload.single('file'), async (req, res) => {
  try {
    if (!req.file || !req.file.buffer) {
      return res.status(400).json({ message: 'Vui lòng tải lên file Excel hợp lệ' });
    }
    ...
    for (const worksheet of workbook.worksheets) {
      ...
      for (let r = headerRowIndex + 1; r <= worksheet.rowCount; r++) {
        ...
        let studentCode = '', fullName = '', dob = '', major = '', phone = '', parentPhone = '', rawCourseGroups = '';

        row.eachCell((cell, colNumber) => {
          const colHeader = (headerMap[colNumber] || '').trim().toLowerCase();
          const cellVal = cell.value !== null && cell.value !== undefined ? String(cell.value).trim() : '';
          ...
          else if (colHeader.includes('sđt sinh viên') || colHeader.includes('sđt sv')) phone = cellVal;
          else if (colHeader.includes('sđt phụ huynh') || colHeader.includes('sđt ph')) parentPhone = cellVal;
        ...
        const courseGroupsArray = rawCourseGroups
          ? rawCourseGroups.split(',').map((s) => s.trim()).filter(Boolean)
          : [];
    ...
    res.status(500).json({ message: 'Lỗi khi xử lý file Excel import' });
  }
});
