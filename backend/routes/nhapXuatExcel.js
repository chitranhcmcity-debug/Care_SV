  ...
  async (req, res, next) => {
    try {
      ...
      for (const worksheet of workbook.worksheets) {
        ...
        // Tìm NhomHocPhan tương ứng với tên sheet
        const courseGroup = await NhomHocPhan.findOne({
        ...
        for (let r = headerRowIndex + 1; r <= lastRow; r++) {
          ...
          try {
            const isNew = !(await SinhVien.findOne({ studentCode }));
            ...
            // Gán vào NhomHocPhan nếu chưa có
            ...
            // Cập nhật courseGroups trong SinhVien
            if (!student.courseGroups || !student.courseGroups.includes(courseGroup.groupCode)) {
              await SinhVien.findByIdAndUpdate(student._id, {
      ...
      for (const worksheet of workbook.worksheets) {
        ...
        for (let r = headerRowIndex + 1; r <= worksheet.rowCount; r++) {
          ...
          for (const gCode of courseGroupsArray) {
            await NhomHocPhan.findOneAndUpdate(
              { groupCode: gCode },
              {
